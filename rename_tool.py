import os

# ================= 1. ตั้งค่าตรงนี้ =================
# โฟลเดอร์ที่เก็บคลิปดิบของคุณ (เปลี่ยน path ให้ถูกนะ)
TARGET_FOLDER = r"data/raw/no_action" 

# ชื่อใหม่ที่อยากตั้ง (เช่น 'fever' จะกลายเป็น fever_0.mp4)
NEW_NAME_PREFIX = "no_action" 

# อยากให้เริ่มนับเลขที่เท่าไหร่? (ปกติเริ่ม 0)
START_INDEX = 0 
# =================================================

def rename_all_files():
    # ตรวจสอบว่ามีโฟลเดอร์ไหม
    if not os.path.exists(TARGET_FOLDER):
        print(f"❌ หาโฟลเดอร์ไม่เจอ: {TARGET_FOLDER}")
        return

    # ดึงชื่อไฟล์ทั้งหมดมา
    files = [f for f in os.listdir(TARGET_FOLDER) if os.path.isfile(os.path.join(TARGET_FOLDER, f))]
    
    # (สำคัญ) เรียงลำดับชื่อเดิมก่อน เพื่อให้การเปลี่ยนชื่อเป็นไปตามลำดับ (ไม่สุ่ม)
    files.sort()

    print(f"📂 พบไฟล์ทั้งหมด {len(files)} ไฟล์ ใน: {TARGET_FOLDER}")
    print("กำลังเปลี่ยนชื่อ...")

    count = 0
    for i, old_filename in enumerate(files):
        # แยกนามสกุลไฟล์ (.mp4, .avi) ออกมา
        _, extension = os.path.splitext(old_filename)
        
        # ถ้าไฟล์ไม่ใช่ video (เช่น .txt, .py) ให้ข้าม (กันไว้ก่อน)
        if extension.lower() not in ['.mp4', '.avi', '.mov', '.mkv']:
            continue

        # สร้างชื่อใหม่
        new_filename = f"{NEW_NAME_PREFIX}_{START_INDEX + i}{extension}"
        
        # Path เต็มของไฟล์เก่าและใหม่
        old_path = os.path.join(TARGET_FOLDER, old_filename)
        new_path = os.path.join(TARGET_FOLDER, new_filename)

        # สั่งเปลี่ยนชื่อ
        os.rename(old_path, new_path)
        print(f"  ✅ {old_filename} -> {new_filename}")
        count += 1

    print(f"\n🎉 เรียบร้อย! เปลี่ยนชื่อไปทั้งหมด {count} ไฟล์")

if __name__ == "__main__":
    rename_all_files()